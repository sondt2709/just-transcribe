...
Defines requirements for always-on structured tracing of the KMP Android transcription pipeline, per-second heartbeats, stall detection, and opt-in audio debug recording, so pipeline stalls and failures can be diagnosed on-device from session artifacts.
