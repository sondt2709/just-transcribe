...
## 4. Backup freshness (android)

- [x] 4.1 Key-value `BackupAgentHelper` with `FileBackupHelper` on the DataStore file; register via `android:backupAgent` in the manifest
- [x] 4.2 Call `BackupManager.dataChanged()` after every settings save

## 5. Verification

- [ ] 5.1 Build debug APK; manual matrix: fresh install shows model defaults; single-model server auto-selects; back with edits prompts; save against reachable server with wrong model warns; save with server down asks confirmation; save → `adb shell bmgr backupnow` → un/reinstall restores last-saved values
