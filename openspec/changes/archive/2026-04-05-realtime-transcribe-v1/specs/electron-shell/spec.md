...
On first launch, the Electron app SHALL verify that required prerequisites are installed: `uv` and `huggingface-cli` (hf). If missing, it SHALL display instructions for installation.

#### Scenario: All prerequisites present
- **WHEN** the app launches and detects `uv` and `hf` CLI in PATH
- **THEN** it proceeds to check Python environment and model status

#### Scenario: uv not installed
- **WHEN** the app cannot find `uv` in PATH
- **THEN** it displays a setup screen with the instruction: `brew install uv` or `curl -LsSf https://astral.sh/uv/install.sh | sh`

#### Scenario: huggingface-cli not installed
- **WHEN** the app cannot find `hf` in PATH
- **THEN** it displays a setup screen with the instruction: `uv tool install huggingface-cli`
...
The Electron app SHALL check if required models are downloaded. If not, it SHALL trigger download via `huggingface-cli download` and display progress.

#### Scenario: Models not downloaded
- **WHEN** the Qwen3-ASR 1.7B model is not found in `~/.cache/huggingface/`
- **THEN** the app displays a model download screen and runs `hf download Qwen/Qwen3-ASR-1.7B` showing progress

#### Scenario: Models already present
- **WHEN** required models exist in cache
- **THEN** the app skips download and proceeds to start
